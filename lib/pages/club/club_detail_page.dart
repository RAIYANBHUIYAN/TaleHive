...
class _ClubDetailPageState extends State<ClubDetailPage> with SingleTickerProviderStateMixin {
  ...
  void _handleBookTap(ClubBook book) {
    if (book.bookId.isNotEmpty) {
      // Navigate to Book Details page with premium club context
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => BookDetailsPage(
            bookId: book.bookId,
            isFromPremiumClub: widget.club.isPremium, // Pass premium status
            clubId: widget.club.id, // Pass club ID
            bookTitle: book.bookTitle ?? 'Unknown Title', // Pass book title
            authorName: null, // Don't pass author name as per requirement
            bookCategory: book.bookCategory, // Pass book category
          ),
        ),
      );
    } else {
